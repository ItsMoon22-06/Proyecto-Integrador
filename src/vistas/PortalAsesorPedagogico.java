...
public class PortalAsesorPedagogico extends JFrame {
    ...
    public PortalAsesorPedagogico(AsesorPedagogico asesor) {
        ...
        profilePanel.add(nameLabel); profilePanel.add(profileCircle);
        ...
        // Práctica con sub-CardLayout (esTutor=false → bitácora solo lectura, sin cerrar)
    ...
    private void buildSidebar() {
        ...
        JPanel header = new JPanel(new BorderLayout()); header.setOpaque(false); header.setBorder(new EmptyBorder(12,15,12,12));
        JPanel titles = new JPanel(); titles.setLayout(new BoxLayout(titles,BoxLayout.Y_AXIS)); titles.setOpaque(false);
        lblSigep  = new JLabel("SIGEP"); lblSigep.setForeground(Color.WHITE); lblSigep.setFont(new Font("SansSerif",Font.BOLD,22));
        lblPortal = new JLabel("Portal Asesor Pedagógico"); lblPortal.setForeground(Color.LIGHT_GRAY); lblPortal.setFont(new Font("SansSerif",Font.PLAIN,12));
        titles.add(lblSigep); titles.add(lblPortal);

        JLabel toggle = new JLabel("\u2261");
        toggle.setFont(new Font("SansSerif",Font.BOLD,26)); toggle.setForeground(Color.WHITE);
        toggle.setCursor(new Cursor(Cursor.HAND_CURSOR)); toggle.setVerticalAlignment(SwingConstants.CENTER); toggle.setHorizontalAlignment(SwingConstants.CENTER);
        header.add(titles, BorderLayout.CENTER); header.add(toggle, BorderLayout.EAST);
        sidebar.add(header, BorderLayout.NORTH);

        JPanel items = new JPanel(); items.setLayout(new BoxLayout(items,BoxLayout.Y_AXIS)); items.setOpaque(false); items.setBorder(new EmptyBorder(10,0,0,0));
        for (int i = 0; i < ICONOS.length; i++) {
            ...
            lbl.addMouseListener(new MouseAdapter() {
                ...
                @Override public void mouseClicked(MouseEvent e) { cardLayout.show(mainContentPanel, card); }
            });
            items.add(lbl); items.add(Box.createRigidArea(new Dimension(0,5)));
        }
        sidebar.add(items, BorderLayout.CENTER);

        JPanel logoutPanel = new JPanel(new BorderLayout()); logoutPanel.setOpaque(false); logoutPanel.setBorder(new EmptyBorder(10,0,20,0));
        ...
        lblLogout.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) {
                ...
                new controlador.LoginControlador(lw); lw.setVisible(true);
                dispose();
            }
        });
    ...
    private void actualizarSidebar() {
        if (sidebarExpanded) {
            sidebar.setPreferredSize(new Dimension(SIDEBAR_EXPANDED, getHeight())); lblSigep.setVisible(true); lblPortal.setVisible(true);
            for (int i = 0; i < menuLabels.size(); i++) { menuLabels.get(i).setText(ICONOS[i]+"   "+TEXTOS[i]); menuLabels.get(i).setBorder(new EmptyBorder(12,25,12,20)); menuLabels.get(i).setHorizontalAlignment(SwingConstants.LEFT); }
            lblLogout.setText("\uD83D\uDEAA   Cerrar Sesión"); lblLogout.setBorder(new EmptyBorder(12,25,12,20)); lblLogout.setHorizontalAlignment(SwingConstants.LEFT);
        } else {
            sidebar.setPreferredSize(new Dimension(SIDEBAR_COLLAPSED, getHeight())); lblSigep.setVisible(false); lblPortal.setVisible(false);
            for (int i = 0; i < menuLabels.size(); i++) { menuLabels.get(i).setText(ICONOS[i]); menuLabels.get(i).setBorder(new EmptyBorder(12,0,12,0)); menuLabels.get(i).setHorizontalAlignment(SwingConstants.CENTER); }
            lblLogout.setText("\uD83D\uDEAA"); lblLogout.setBorder(new EmptyBorder(12,0,12,0)); lblLogout.setHorizontalAlignment(SwingConstants.CENTER);
        }
    ...
    private JLabel crearEtiquetaMenu(String icono, String texto) {
        ...
        label.setForeground(Color.WHITE); label.setFont(new Font("SansSerif",Font.PLAIN,14));
        label.setBorder(new EmptyBorder(12,25,12,20)); label.setMaximumSize(new Dimension(Integer.MAX_VALUE,45));
        label.setCursor(new Cursor(Cursor.HAND_CURSOR)); return label;
    }
